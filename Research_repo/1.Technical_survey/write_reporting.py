...
def _read_dataframe_from_csv_on_s3(path, dataset_name, sep):
    ...
    #with fs.open(os.environ['S3_BUCKET'] + ) as f:
    df = pd.read_table(f'{path}/{dataset_name}', sep=sep)
    return df
        ...
        if cross_val_type == "skfold":
            ...
            for name, model in models:
                ...
                for train_index, test_index in skf.split(X, Y):
                    # begin
                    start = time.time()
                    print("je suis bien dasn skfold")
                    if smote == False:
                        # split the data
                        ...
                        # sm = ADASYN(random_state=152)
                        pipeline = SMOTETomek(random_state=152)
                        ...
                        # to test 1
                        #pipeline = NeighbourhoodCleaningRule()
                        print("I am using the SMOTE + Tomek")
